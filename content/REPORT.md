...
差分プライバシー (DP) 下の線形回帰では、加えたノイズが二乗和行列にバイアスを持ち込み、
素朴な推定・素朴な分散推定はいずれも歪む。論文 [1] は **Binning-Aggregation (BinAgg)**
により、(i) バイアス補正済みの点推定、(ii) サンドイッチ分散による**漸近的に妥当な信頼区間**、
(iii) 同一予算で生成でき回帰の保証を満たす合成データ、を与えると主張する。

本追試では公式実装 [2] を固定環境で動かし、論文の主眼である「妥当な信頼区間」と
「回帰分析の保証で測る合成データ utility」を 3 実験で定量検証した。**結論として主張は再現された**:
シミュレーションで BinAgg の 95% CI 被覆率は μ 全水準で 0.92–0.93（公称 0.95 に近接）に対し、
ノイズを無視する素朴 CI は 0.08–0.32 に崩壊した (E1)。合成データへ素朴に回帰した係数の相対
L2 誤差 0.58→0.06 に対し、BinAgg の補正推定は 0.09→0.02 と非プライベート OLS (0.003) に接近
した (E2)。実データ (UCI Air Quality) でも CI 幅・合成モーメント誤差が μ とともに単調に縮小し、
プライバシー–精度トレードオフが期待どおり観測された (E3)。あわせて、同梱データに残る欠損値
センチネル `-200` がベンチ手順を汚染する再現上の落とし穴を特定した。
