...
(4) **Wine の乖離の原因切り分けと解消**（§5）、(5) **係数レベルの比較**（§6）、
(6) **合成データの下流 ML 評価（論文 §5.2.2 / Table 3）**（§7）を best-effort でまとめる。
(1)–(5) は論文 Table 2、(6) は Table 3 の報告値と並べて評価する。
...
- 比較手法（AdaSSP, §1）・他データ（Abalone/Wine/Appliances, §2）・反復 100（§3）・**下流 ML 評価（§5.2.2, §7）**の
  いずれでも、BinAgg は論文と整合する挙動を示した。特に予測精度の優劣・桁・劣化の向き・データ間の相対関係を再現できた。
...
- 残る発展: **§5.2.2 の競合 DP-SDG 手法（AIM 等）の実装**による「BinAgg が最良」主張の検証、DP-GD の追加、
  **唯一未同定の D3 (2043, 8)** の特定、係数レベル評価の安定化（[補足実験](supplementary.html)の標準化参照）。
