...
### Q. E3 で実データの DP 係数が非プライベート OLS と一致しないのは手法が悪いから?

いいえ。主因はデータ側にあります。Air Quality データの欠損センチネル `-200` が OLS 参照側を汚染しており、一方 DP 経路は bounds でクリップした後の値を見ます。つまり両者はそもそも別物を比較している状態です。

したがって、本データでは「DP-OLS 係数の一致」は妥当な有用性指標になりません。CI 幅や、クリップ後を基準とした合成モーメント誤差の方が信頼できる指標と考えられます（詳細は「前処理とドメイン特化メモ」を参照）。
